...
def main(argv):
  ...
  if solution_state:
    ...
    (data, size) = solution_state.posix.fd[sys.stdin.fileno()].read_data(20)
    for byte in data.chop(bits=8):
        alphanumeric_constraint = claripy.Or(
            claripy.And(byte == 0x0),
            claripy.And(byte >= 0x41, byte <= 0x5a),
            claripy.And(byte >= 0x61, byte <= 0x7a),
            claripy.And(byte >= 0x30, byte <= 0x39)
        )
        solution_state.add_constraints(alphanumeric_constraint)
