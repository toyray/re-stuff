...
class Disasm:
    ...
    def load(self, filename):
        ...
        self.create_copy_with_new_section(self.filename, self.new_filename)
        self.f = open(self.filename, "rb")
        self.pe = pefile.PE(self.new_filename, fast_load=True)
        self.new_code_start = self.get_new_code_start()
    ...
    def create_copy_with_new_section(self, src, dest):
        """
        Make a copy of PE file with a new section to store the reconstructed
        code
        """
        binary = lief.parse(src)
        section = lief.PE.Section(".new")
        section.content = [0x00] * self.max_func_size * self.max_funcs
        binary.add_section(section, lief.PE.SECTION_TYPES.TEXT)
        builder = lief.PE.Builder(binary)
        builder.build()
        builder.write(dest)

    def get_new_code_start (self):
        """
        Find the beginning virtual address of new section
        """
        for section in self.pe.sections:
            if section.Name == b".new\x00\x00\x00\x00":
                return section.VirtualAddress + self.pe.OPTIONAL_HEADER.ImageBase

        print("new code section not found, aborting")
        exit()

    def wipe_existing_code(self):
        """
        Wipes the code in existing .text section to remove all those annoying code
        references due to the old obfuscated jump funcs
        """
        for section in self.pe.sections:
            if section.Name == b".text\x00\x00\x00":
                with open(self.new_filename, "rb+") as f:
                    f.seek(section.PointerToRawData)
                    f.write(b"\xc3" * section.SizeOfRawData)
                break
    ...
    def start(self, va, single_func=False):
        ...
        while True:
            ...
            if nva not in proc_funcs:
                ...
                # Add new functions/jumps to process
...
if __name__ == "__main__":
    cfg = {
        ...
        "max_insn": 9000,
        "patch_ext": ".patched",
        # TODO: This are both arbitrary values selected after previous runs
        # with the script. A dynamically expanding new code section would be
        # more elegant
    ...
    # Test a single function with only conditional jumps to duplicate code
    # d.start(0x1800027b8,True)

    # Test a single function with small unrolled loop
    # d.start(0x18000647e, True)

    # Test a single function with both conditional jumps and unrolled loop
    # d.start(0x180006d53,True)

    # Test a single function with jmp rax instead of ret
    # d.start(0x18006080b, True)
